...
import scss from '../../styles/utils/theme.module.scss'
...
export default function SetTheme() {
	...
	useEffect( () => {
		...
		/**
		 * This function is for the eventListeners only. When this function is fired
		 * This causes a cascade effect through the 
		 * @param {*} e 
		 */
		function useSetTheme(e)	{
			console.log('useSetTheme')
			console.log(e)
		...
		// Check what theme the system is set to
		let themeSystem =  window.matchMedia('(prefers-color-scheme: dark)').matches ? 'dark' : 'light'
		// Set the default theme: if a theme value is stored in user storage use that, if not use the system default.
		let themeDefault = themeLocalStorage ?? themeSystem
		
		// Set the current theme to root, and use a theme data attribute to store the state value
		// If a theme has been selected use that, otherwise use the default option
		document.querySelector(':root').dataset.theme = theme ?? themeDefault
		setTheme( theme ?? themeDefault )
		console.log('useEffect( () => { setTheme')

		// If a theme has been selected by the user set it in localStorage so it will be remember next time
		// ? Will localStorage persist between long term sessions, is a cookie more viable?
	...
	return (
		<>
		<span key="themeToggle" onClick={toggleTheme} data-theme={theme} className={scss.toggle}>{buttonIcon(theme)}</span>
		</>
	)
}